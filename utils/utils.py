...
def plot_error(data: pd.DataFrame, out_dir: Path) -> None:
    ...
    samples = np.logspace(2, 6, dtype=np.int32)
    data = data.query('samples in @samples')
    sns.lineplot(x="samples", y="error", hue="algorithm", data=data, ax=ax)
    plt.xlabel("Number of samples")
    plt.ylabel("$|\mathcal{H}_{traj}(\pi^{\star}) - \mathcal{H}_{traj}(\pi^{t}) |$")
...
def plot_1d_occupancies(data: pd.DataFrame, out_dir: Path) -> None:
    #hue_order=['Random Policy', 'MVEE Policy', 'EntGame', 'MTEE Policy', 'UCBVI-Ent']
    sns.lineplot(x="state", y="occupancy", hue_order=hue_order, hue="algorithm", data=data, dashes=(2,2))
    plt.yscale("log")
    plt.xlabel("State $s$")
...
def plot_2d_occupancies(data: pd.DataFrame, env: GridWorld, out_dir: Path) -> None:
    ...
    for i, algorithm in enumerate(algorithms):
        ...
        im = ax.imshow(occupancies,
            norm=colors.SymLogNorm(linthresh=1000, linscale=1, vmin=0, vmax=v_max),
...
def softmax_sample(x: np.ndarray) -> int:
    """
        Sample from softmax(x)
    :param x: an 1d array
    :return: a random index from softmax(x)
    """
    softmax = np.exp(x - np.max(x))
    softmax /= softmax.sum()
    return np.random.choice(x.shape[0], p=softmax)
